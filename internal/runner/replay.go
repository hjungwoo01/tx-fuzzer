...
func RunReplay(ctx context.Context, env *Env) error {
	...
	for idx, step := range replayCfg.Schedule {
		...
		case "step":
			...
			count := step.Steps
			if count <= 0 {
				count = len(state.plan.Template.Steps) - state.stepIndex
			}
			if count <= 0 {
				continue
			}
	...
	for alias, state := range states {
		...
		if state.stepIndex >= len(state.plan.Template.Steps) {
			if err := state.commit(env); err != nil {
...
func (s *replayTxnState) ensureStarted(parent context.Context, env *Env) error {
	...
	s.tx = tx
	s.stepVals = nil
	s.invokeMops = make([]history.Mop, 0, len(s.plan.Template.Steps))
	s.okMops = make([]history.Mop, 0, len(s.plan.Template.Steps))
	s.stepIndex = 0
	s.firstMono = 0
	s.firstWall = 0
	s.lastMono = 0
...
func (s *replayTxnState) runSteps(env *Env, count int) error {
	for i := 0; i < count && s.stepIndex < len(s.plan.Template.Steps); i++ {
...
func (s *replayTxnState) runSingleStep(env *Env) error {
	...
	st := s.plan.Template.Steps[s.stepIndex]
	gen := s.plan.ArgsPerStep[s.stepIndex]
	...
	ctx := s.ctx
	if hasSelect(st.SQL) {
		row, err := s.tx.QueryRow(ctx, st.SQL, args...)
		if err != nil {
			return s.fail(env, stepStartMono, stepStartWall, fmt.Errorf("query step %d failed: %w", s.stepIndex, err))
		}
		var scanVal any
		if st.Elle != nil && st.Elle.ValueFromResultCol != nil {
			if err := row.Scan(&scanVal); err != nil {
				return s.fail(env, stepStartMono, stepStartWall, fmt.Errorf("scan step %d failed: %w", s.stepIndex, err))
			}
		} else {
			var dummy any
		...
		if _, err := s.tx.Exec(ctx, st.SQL, args...); err != nil {
			return s.fail(env, stepStartMono, stepStartWall, fmt.Errorf("exec step %d failed: %w", s.stepIndex, err))
		}
		if haveElle {
			val := st.Elle.Value
			if val == nil {
...
func (s *replayTxnState) fail(env *Env, stepStartMono, stepStartWall int64, cause error) error {
	if s.tx != nil {
	...
	if stepStartMono != 0 {
		s.lastMono = stepStartMono
	} else {
		s.lastMono = time.Since(env.Start).Nanoseconds()
	}
	if stepStartWall != 0 {
		s.lastWall = stepStartWall
	} else {
		s.lastWall = time.Now().UnixNano()
	}
...
func (s *replayTxnState) emitHistory(env *Env, result history.OpType) {
	...
	recordTxnOp(env.Hist, history.Invoke, s.process, s.firstMono, s.firstWall, s.invokeMops, s.plan.Template.Name)
	recordTxnOp(env.Hist, result, s.process, s.lastMono, s.lastWall, s.okMops, s.plan.Template.Name)
}
